...
(
    ...
    # ADD_ACCOUNT flow
    GET_ACCOUNT_DETAILS,        # Ask for account name/details (e.g. Netflix acc@ex.com) [cite: 2]
    GET_ACCOUNT_SERVICE,        # Ask for service for this new account (e.g. Netflix) [cite: 2]
    ...
    # SET_PRICES flow
    PRICE_MAIN_MENU,            # Main menu: Add/Edit Service, View Services, Remove Price Option [cite: 3]
    PRICE_GET_SERVICE_NAME,     # Enter name of service to add/edit [cite: 3]
    PRICE_GET_EMOJI,            # Enter emoji for service [cite: 3]
    PRICE_GET_DURATION_DAYS,    # How many days for this pricing option? [cite: 3]
    PRICE_GET_PRICE_AMOUNT,     # What's the price for this duration? [cite: 4]

    # REMOVE_PRICE_OPTION flow (New sub-flow for set_prices) [cite: 5]
    PRICE_REMOVE_SELECT_SERVICE, # Select service to remove price from [cite: 5]
    PRICE_REMOVE_SELECT_DURATION,# Select duration to remove [cite: 5]

    # REMOVE_SUBSCRIPTION flow (/removesub command)
    REMSUB_CHOOSE_PERSON,       # User has typed /removesub, bot asks which person [cite: 5]
    REMSUB_CHOOSE_SUBSCRIPTION, # Person selected, bot asks which of their subs [cite: 5]
    REMSUB_CONFIRM_DELETION     # Subscription selected, confirm deletion [cite: 5]
) = range(20)
...
def load_data():
    if not os.path.isfile(DATA_FILE):
        data = {
            "people": {},  # person_name -> {"subscriptions": [], "last_active": str} [cite: 6]
            "accounts": {},  # account_name -> {"service": str, "slots": {slot_num: person_name or None}} [cite: 6]
            "services": {},  # service_name -> {"emoji": str, "durations": {days: price}} [cite: 6]
            "default_slots": {} # service_name -> count [cite: 6]
        }
        save_data(data)
    else: # [cite: 7]
        with open(DATA_FILE, "r") as f:
            data = json.load(f)
    # Ensure default_slots key exists [cite: 7]
...
def cleanup_expired_subs():
    ...
    Also removes inactive people (no subs, inactive > 10 days). [cite: 8, 9]
    """
    data = load_data()
    now = datetime.utcnow()
    changed = False
    ...
    for person_name in list(data["people"].keys()):
        ...
        for sub in person.get("subscriptions", []):
            try:
                ...
                if (now - end_date).days <= 60: # Keep sub if not expired by more than 60 days [cite: 10]
                    new_subs.append(sub)
                else: # Subscription expired more than 60 days ago [cite: 10]
                    logger.info(f"Subscription for {person_name} ({sub.get('service')}) expired on {sub['end_date']}, removing.")
                    # Free the slot if exists [cite: 11]
                    account = data["accounts"].get(sub["account"])
                    if account:
                        slot_num = sub.get("slot")
                        if slot_num is not None and account["slots"].get(str(slot_num)) == person_name: # [cite: 12]
                            account["slots"][str(slot_num)] = None
                            logger.info(f"Freed slot {slot_num} in account {sub['account']} from {person_name}.")
                            changed = True # [cite: 13]
                    changed = True # because sub is removed [cite: 13]
            except ValueError: # Catch issues with date parsing, keep sub if unsure [cite: 13]
                logger.error(f"Could not parse end_date for subscription: {sub}")
                new_subs.append(sub) # [cite: 14]

        if len(person["subscriptions"]) != len(new_subs):
            person["subscriptions"] = new_subs
            changed = True

        # Remove person if no subscriptions for more than 10 days (inactive) [cite: 14]
        if (
            ...
            try:
                ...
                if (now - last_active_date).days > 10:
                    ...
                    del data["people"][person_name] # [cite: 16]
                    changed = True
            except ValueError:
...
Let's make managing shared accounts *easy peasy*! [cite: 18]
...
""" # [cite: 19]
    keyboard_buttons = [
        ...
        [InlineKeyboardButton("📊 List Subscriptions", callback_data="list_subs_cmd")], # [cite: 20]
    ]
    ...
    return ConversationHandler.END # Ensure start can terminate other convos if user hits /start [cite: 20]
...
async def handle_list_subs_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.answer()
    await list_subscriptions(update, context, from_callback=True) # [cite: 21]


# Add Person handlers
async def add_person_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed from add_person_start [cite: 21]
    await update.callback_query.answer()
    await update.callback_query.edit_message_text("👋 Who's joining the subscription party? Send me their name:") # [cite: 22]
...
async def add_person_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await start(update, context) # Go back to main menu [cite: 23]
    return ConversationHandler.END


# Remove Person handlers
async def remove_person_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed from remove_person_start [cite: 23]
    data = load_data()
    await update.callback_query.answer()
    if not data["people"]:
    ...
    await update.callback_query.edit_message_text("Who's leaving us? 😢 Pick someone:", reply_markup=keyboard) # [cite: 24]
...
async def remove_person_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = load_data()
    if person_name in data["people"]:
        # Also remove person from any slots they occupy [cite: 24]
        for acc_name, acc_details in data["accounts"].items():
            for slot_num, occupant in list(acc_details["slots"].items()): # Use list() for safe iteration [cite: 24]
                if occupant == person_name: # [cite: 25]
        ...
        save_data(data)
        await query.edit_message_text(f"👋 Farewell, {person_name}! I've removed them and freed up their slots.") # [cite: 26]
    else:
        await query.edit_message_text("🤨 Hmm, I can't find that person. Maybe they already left?") # [cite: 27]
    await start(update, context)
    return ConversationHandler.END
...
async def list_people(update: Update, context: ContextTypes.DEFAULT_TYPE, from_callback=False):
    ...
    for person, info in data["people"].items(): # [cite: 28]
        ...
        if not info["subscriptions"]:
            ...
            for sub in info["subscriptions"]:
                text += (
                    f"  - {sub['service']} on {sub['account']} (Slot {sub.get('slot', '-')}) " # [cite: 29]
                    f"until {sub['end_date']} ({sub['duration']} days) - Price: ${sub.get('price', 'N/A')}\n"
                )
    await reply_fn(text, parse_mode="Markdown")
    if from_callback: await start(update, context) # Option to go back to menu if called from button [cite: 29]


# Add Account handlers
async def add_account_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 29]
    await update.callback_query.answer() # [cite: 30]
    await update.callback_query.edit_message_text("🔐 Let's add a new account! What's the account identifier? (e.g. 'Netflix Main' or 'spotify_user@example.com')") # [cite: 31]
    return GET_ACCOUNT_DETAILS


async def add_account_get_details(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 31]
    ...
    if account_name in data["accounts"]:
        ...
        return GET_ACCOUNT_DETAILS # Ask again [cite: 31]

    context.user_data["new_account_name"] = account_name
    
    # Ask for service for this account [cite: 31]
    services = list(data["services"].keys())
    if not services: # [cite: 32]
        ...
        # Temporarily save account with no service, or guide user better [cite: 32]
        data["accounts"][account_name] = {"service": None, "slots": {}} # Save with no service for now [cite: 32]
        save_data(data)
        await update.message.reply_text(f"Account '{account_name}' added, but no service linked. You might need to manage this manually or add services first.") # [cite: 33]
        await start(update, context)
        return ConversationHandler.END
...
async def add_account_set_service(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 34]
    query = update.callback_query
    await query.answer()
    ...
    # Populate with default slots if they exist for this service [cite: 35]
    ...
    save_data(data)
    await query.edit_message_text(f"✨ Perfect! Account '{account_name}' is now linked to {service_name} and default slots (if any) have been added.") # [cite: 36]
    await start(update, context)
    return ConversationHandler.END


# Remove Account handlers
async def remove_account_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 36]
    data = load_data()
    await update.callback_query.answer()
    ...
    buttons = [(f"🔑 {name} ({data['accounts'][name].get('service', 'N/A')})", f"remove_account_{name}") for name in data["accounts"].keys()] # [cite: 37]
...
async def remove_account_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    account_name = query.data.replace("remove_account_", "") # Ensure this matches button callback_data [cite: 37]
    data = load_data()
    if account_name in data["accounts"]:
        # Check if account has active subscriptions [cite: 37]
        active_subs_on_account = False
        for person_info in data["people"].values(): # [cite: 38]
        ...
        if active_subs_on_account:
            await query.edit_message_text(f"⚠️ Account '{account_name}' still has active subscriptions! Please remove those subscriptions first before deleting the account.") # [cite: 40]
        else:
...
# Add Subscription handlers (Person -> Service -> Account -> Slot -> Duration) [cite: 41]
async def add_sub_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 41]
    data = load_data()
    await update.callback_query.answer()
    if not data["people"]:
    ...
    keyboard = InlineKeyboardMarkup(build_menu(buttons, 2))
    await update.callback_query.edit_message_text("Who's getting a new subscription? 🎁", reply_markup=keyboard) # [cite: 42]
...
async def add_sub_person_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [(f"{data['services'][s]['emoji']} {s}", f"addsub_service_{s}") for s in data["services"].keys()] # [cite: 43]
    keyboard = InlineKeyboardMarkup(build_menu(buttons, 2))
...
async def add_sub_service_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Filter accounts by the chosen service [cite: 43]
    filtered_accounts = {name: acc_info for name, acc_info in data["accounts"].items() if acc_info.get("service") == service_name}

    if not filtered_accounts: # [cite: 44]
        await query.edit_message_text(f"😲 No accounts found for the service '{service_name}'. Add an account for this service first.") # [cite: 45]
        await start(update, context) # Go back to main menu [cite: 45]
        return ConversationHandler.END

    buttons = [(f"🔑 {name}", f"addsub_account_{name}") for name in filtered_accounts.keys()]
    keyboard = InlineKeyboardMarkup(build_menu(buttons, 1)) # 1 column for account names often better [cite: 45]
...
async def add_sub_account_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    account_name = query.data.replace("addsub_account_", "") # [cite: 46]
    ...
    # Show free slots (slots that are None) [cite: 46]
    free_slots = {num: holder for num, holder in account.get("slots", {}).items() if holder is None}
    
    if not account.get("slots"): # No slots defined at all for this account [cite: 47]
         await query.edit_message_text(f"😫 Account '{account_name}' has no slots defined. You can add slots using /add_slot {account_name} <slot_number>.") # [cite: 48]
         await start(update, context)
         return ConversationHandler.END
    elif not free_slots: # Slots are defined, but all are taken [cite: 48]
    ...
    keyboard = InlineKeyboardMarkup(build_menu(buttons, 3)) # 3 slots per row might be good [cite: 49]
...
async def add_sub_slot_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["addsub_slot"] = slot_num_str # Keep as string, keys in JSON are strings [cite: 49]
    ...
    if not durations:
        await query.edit_message_text(f"🤷 No durations (and prices) set for the service '{service_name}'. Please use 'Set Prices' from the main menu to set them up first!") # [cite: 50]
        await start(update, context)
        return ConversationHandler.END
...
async def add_sub_duration_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    subscription = {
        ...
        "slot": slot_num_str, # Keep as string to match dict keys if they are string numbers [cite: 52]
        "duration": duration_days,
        "end_date": end_date, # [cite: 53]
        "price": price,
    }
    ...
    if account_name in data["accounts"] and slot_num_str in data["accounts"][account_name]["slots"]:
        ...
        await query.edit_message_text("Error: Account or slot not found during finalization. Please check data integrity.") # [cite: 54]
        await start(update, context)
        return ConversationHandler.END

    # Add to person's subscriptions [cite: 54]
...
Enjoy! 🍿🎶""", parse_mode="Markdown"
    ...
    await start(update, context) # [cite: 55]
    return ConversationHandler.END


# Set Prices handlers
async def set_prices_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 55]
    await update.callback_query.answer()
    buttons = [
        ("✨ Add/Edit Service Price", "setprice_add_edit_service"),
        ("📜 View Services & Prices", "setprice_view_services"),
        ("🗑️ Remove Price Option", "setprice_remove_option_start"), # New button [cite: 55]
        ("🔙 Back to Menu", "main_menu_from_prices"),
    ]
    keyboard = InlineKeyboardMarkup(build_menu(buttons, 1))
    await update.callback_query.edit_message_text("💰 *Price Management* 💰\nWhat would you like to do?", reply_markup=keyboard, parse_mode="Markdown") # [cite: 56]
    return PRICE_MAIN_MENU


async def price_main_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 56]
    query = update.callback_query
    await query.answer()
    action = query.data
    ...
    if action == "setprice_view_services":
        ...
        if not data["services"]:
            ...
            await query.edit_message_text(text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Price Menu", callback_data="set_prices_start_dummy")]])) # Dummy to re-trigger prev menu [cite: 58]
        return PRICE_MAIN_MENU # Or ConversationHandler.END then user clicks back if needed [cite: 59]
        ...
        if not services_with_prices:
            ...
            return PRICE_MAIN_MENU # [cite: 61]
    ...
    elif action == "set_prices_start_dummy": # From view services back button [cite: 61]
        return await set_prices_start_cb(update, context) # Re-show price menu [cite: 61]

    return PRICE_MAIN_MENU # Default return if no specific action taken to end/redirect [cite: 62]


async def price_get_service_name_receive(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 62]
    service_name = update.message.text.strip()
    context.user_data["price_service_name"] = service_name
    data = load_data()

    current_emoji = "❓" # Default emoji [cite: 62]
    if service_name not in data["services"]:
        data["services"][service_name] = {"emoji": current_emoji, "durations": {}}
        # No save_data(data) here, save when emoji or price is set. [cite: 62]
    else: # [cite: 63]
...
async def price_get_emoji_receive(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 63]
    emoji = update.message.text.strip()
    service_name = context.user_data.get("price_service_name")
    data = load_data()

    if service_name: # Should always exist if flow is correct [cite: 64]
        # Ensure service entry exists [cite: 64]
        if service_name not in data["services"]:
            data["services"][service_name] = {"emoji": emoji, "durations": {}}
        else:
            data["services"][service_name]["emoji"] = emoji
        save_data(data)
        await update.message.reply_text(f"👌 Emoji {emoji} saved for {service_name}! Now, for how many days is this pricing option? (e.g., 30)") # [cite: 65]
    else:
...
async def price_skip_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = load_data()
    if service_name:
        if service_name not in data["services"]: # If service is new and emoji skipped [cite: 65]
             data["services"][service_name] = {"emoji": "❓", "durations": {}} # [cite: 66]
             save_data(data) # Save the new service with default emoji [cite: 66]
        # If service exists, its current emoji is kept, or default '❓' if none. [cite: 66]
    await update.message.reply_text("Keeping current/default emoji. Now, for how many days is this pricing option? (e.g., 30)")
    return PRICE_GET_DURATION_DAYS


async def price_get_duration_days_receive(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 66]
    duration_str = update.message.text.strip()
    if not duration_str.isdigit() or int(duration_str) <= 0:
         await update.message.reply_text("🙅‍♂️ Oops! Duration must be a positive number. Try again:") # [cite: 67, 68]
        return PRICE_GET_DURATION_DAYS # Ask for duration again [cite: 68]
...
async def price_get_amount_receive(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 68]
    ...
    try:
        ...
        if price_val < 0: raise ValueError("Price cannot be negative")
         ...
         await update.message.reply_text("💸 That doesn't look like a valid positive price. Try again (e.g., 9.99):") # [cite: 69]
        return PRICE_GET_PRICE_AMOUNT # Ask for price again [cite: 69]

    service_name = context.user_data.get("price_service_name")
    duration_days = context.user_data.get("price_duration_days")

    if not service_name or duration_days is None:
        await update.message.reply_text("🤯 Whoops! Something went wrong (missing service or duration). Let's start setting prices over.") # [cite: 70]
        # Ideally, guide back to set_prices_start_cb or main menu [cite: 70]
        await start(update, context)
        return ConversationHandler.END
        
    data = load_data()
    # Ensure service and durations dict exists [cite: 70]
    if service_name not in data["services"]:
        data["services"][service_name] = {"emoji": "❓", "durations": {}} # Should have been created earlier [cite: 70]
    if "durations" not in data["services"][service_name]: # [cite: 71]
         data["services"][service_name]["durations"] = {}

    data["services"][service_name]["durations"][str(duration_days)] = price_val # Store duration as string key [cite: 71]
    save_data(data)
...
Use 'Set Prices' menu to add more options or go back to main menu.""", parse_mode="Markdown"
    ...
    # Offer to go back to price menu or main menu [cite: 71]
    keyboard = [[InlineKeyboardButton("🔙 Price Menu", callback_data="set_prices_start_dummy_end")],[InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu_from_price_set_end")]] # [cite: 72]
    await update.message.reply_text("What next?", reply_markup=InlineKeyboardMarkup(keyboard))
    return PRICE_MAIN_MENU # Go back to price menu options [cite: 72]
...
async def price_remove_select_service_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not durations:
        ...
        return PRICE_MAIN_MENU # Back to price menu [cite: 73]
...
async def price_remove_select_duration_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    duration_to_remove = query.data.replace("remprice_dur_", "") # This is a string [cite: 73]
    ...
    if service_name and service_name in data["services"] and \
       ...
       duration_to_remove in data["services"][service_name]["durations"]: # [cite: 74]
        ...
        await query.edit_message_text("🤯 Oops! Could not find that price option to remove. It might have already been deleted.") # [cite: 75]
    
    # Clean up context [cite: 75]
    context.user_data.pop("removeprice_service", None)
    context.user_data.pop("removeprice_duration", None) # if it was set [cite: 75]

    # Go back to price menu [cite: 75]
    # To avoid callback issues, just send a new message for price menu or end [cite: 75]
    # For simplicity, let's end and they can click Set Prices again or /start [cite: 75]
    await query.message.reply_text("Price removal process finished. You can manage other prices or return to the main menu.", # [cite: 76]
                                   reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💰 Set Prices", callback_data="set_prices_start")],
                                                                    [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu_generic")]])) # [cite: 77]
    return ConversationHandler.END # End this specific sub-flow [cite: 77]

async def main_menu_generic_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await start(update, context) # This will edit the message to the main menu [cite: 77]
    return ConversationHandler.END
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cancel_message = "🚫 Operation cancelled. No changes made. Returning to the main menu." # [cite: 78]
    ...
    if update.message:
        ...
        except Exception as e: # If message cannot be edited (e.g. too old) [cite: 78]
            logger.warning(f"Could not edit message on cancel: {e}")
            if update.effective_chat: # Send a new message if edit fails and chat context exists [cite: 79]
                 await context.bot.send_message(chat_id=update.effective_chat.id, text=cancel_message)

    # Clear any user_data specific to the cancelled conversation [cite: 79]
    # Example keys, expand as needed [cite: 79]
    keys_to_clear = ["new_account_name", "addsub_person", "addsub_service", "addsub_account", 
                     ...
                     "removeprice_service", "remove_sub_person"] # [cite: 80]
    for key in keys_to_clear:
        if key in context.user_data:
            del context.user_data[key]
            
    await start(update, context) # Attempt to show the main menu again [cite: 80]
    return ConversationHandler.END
...
async def list_subscriptions(update: Update, context: ContextTypes.DEFAULT_TYPE, from_callback=False):
    data = load_data() # [cite: 81]
    reply_fn = update.callback_query.edit_message_text if from_callback else update.message.reply_text
    
    if not data.get("people"):
        await reply_fn("🌌 It's quiet... Too quiet. No people with subscriptions found!") # [cite: 82]
        if from_callback: await start(update,context)
        return
    ...
    if from_callback: await start(update, context) # Go back to main menu [cite: 85]


async def calculate_income(update: Update, context: ContextTypes.DEFAULT_TYPE):
    data = load_data()
    total_income_from_active_subs = 0
    # This calculation assumes prices are for the full duration of the sub. [cite: 86]
    # For a more accurate "monthly" income, you'd need to prorate or track payments. [cite: 86]
    # This will sum up the 'price' field of all currently active subscriptions. [cite: 87]
    active_subs_details = [] # [cite: 88]

    for person_name, person_data in data["people"].items():
        for sub in person_data.get("subscriptions", []):
            # Assuming 'price' in subscription is the total price for that subscription period [cite: 88]
            try:
                price = float(sub.get("price", 0))
                total_income_from_active_subs += price
                # For breakdown: [cite: 89]
    ...
    # Note: The original calculation for per month/year was a bit simplistic. [cite: 90]
    # True recurring income is complex. This just sums current active sub values. [cite: 90, 91]
    # If you want estimated *potential* income based on all prices set, that's different. [cite: 91]
    # This version calculates sum of prices of *active* subscriptions. [cite: 92]
    breakdown_text = "\n".join(active_subs_details) if active_subs_details else "No specific subscriptions to list." # [cite: 93]
    await update.message.reply_text( # [cite: 94]
    ...
    if os.path.exists(DATA_FILE):
        try:
            await update.message.reply_document(
                document=open(DATA_FILE, "rb"), # [cite: 95]
            ...
            await update.message.reply_text("🤖 Oops! Something went wrong while trying to send the data file.") # [cite: 96]
    else:
...
async def set_default_slots(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if service_name_arg exists in data["services"] [cite: 97]
    ...
    try:
        count = int(count_str)
        if count < 0:
            await update.message.reply_text("🔢 Slot count must be a non-negative number. Try again!") # [cite: 98]
            return
        ...
        if count == 0:
            ...
            await update.message.reply_text(f"✅ Default slots for *{service_name_arg}* set to *{count}*. New accounts for this service will start with this many slots.", parse_mode="Markdown") # [cite: 99]
    except ValueError:
...
async def add_slot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args or len(context.args) < 2: # Allow multiple slots to be added [cite: 99]
    ...
    if account_name_arg not in data["accounts"]:
        await update.message.reply_text(f"🔍 Account '{account_name_arg}' not found. Check your spelling or use 'Add Account' first!") # [cite: 101]
        return
    ...
    for slot_str in slot_numbers_to_add:
        if not slot_str.isalnum(): # Basic check, could be more specific if slots have a format [cite: 101]
            invalid_slots.append(slot_str)
            continue
        if slot_str in data["accounts"][account_name_arg]["slots"]: # [cite: 102]
    ...
    if already_exist_slots:
        await update.message.reply_text(f"🤔 Slot(s) *{', '.join(already_exist_slots)}* already exist for *{account_name_arg}*.", parse_mode="Markdown") # [cite: 103]
    if invalid_slots:
        await update.message.reply_text(f"⚠️ Slot name(s) *{', '.join(invalid_slots)}* seem invalid. Please use alphanumeric names.", parse_mode="Markdown") # [cite: 104]
...
async def remove_slot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for slot_str in slot_numbers_to_remove:
        ...
        elif data["accounts"][account_name_arg]["slots"][slot_str] is not None: # [cite: 106]
    ...
    if removed_slots:
        save_data(data)
        await update.message.reply_text(f"🗑️ Slot(s) *{', '.join(removed_slots)}* removed from *{account_name_arg}*.", parse_mode="Markdown") # [cite: 107]

    if not_found_slots:
        await update.message.reply_text(f"🤷‍♂️ Slot(s) *{', '.join(not_found_slots)}* don't exist in *{account_name_arg}*.", parse_mode="Markdown")
    if occupied_slots:
        await update.message.reply_text(f"⚠️ Can't remove occupied slot(s): *{', '.join(occupied_slots)}*. Remove their subscriptions first.", parse_mode="Markdown") # [cite: 108]
...
async def remove_sub_start_command(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 108]
    data = load_data()
    ...
    if not active_people_with_subs:
        await update.message.reply_text("🌌 It's empty here... No people with active subscriptions to remove from!") # [cite: 109]
        return ConversationHandler.END
    
    buttons = [[InlineKeyboardButton(f"👤 {name}", callback_data=f"remsub_person_{name}")]
               for name in active_people_with_subs.keys()]
    await update.message.reply_text("Whose subscription should we remove? Pick a person:", # [cite: 110]
                                  reply_markup=InlineKeyboardMarkup(buttons))
    return REMSUB_CHOOSE_PERSON


async def remove_sub_person_selected(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 110]
    query = update.callback_query
    await query.answer()
    ...
    if person_name not in data["people"] or not data["people"][person_name].get("subscriptions"):
         await query.edit_message_text(f"🤷‍♀️ {person_name} has no subscriptions to remove or person not found!") # [cite: 111]
        context.user_data.pop("remove_sub_person", None) # Clean up context [cite: 111]
        return ConversationHandler.END # Or back to REMSUB_CHOOSE_PERSON if desired [cite: 111]
    
    subs = data["people"][person_name]["subscriptions"]
    
    buttons = []
    for i, s in enumerate(subs):
        # Using index 'i' for callback data to identify the sub in the list [cite: 111]
        buttons.append([InlineKeyboardButton(
            f"{s.get('service','Unknown Service')} on {s.get('account','Unknown Acc')} (Slot {s.get('slot','N/A')}), ends {s.get('end_date','N/A')}", # [cite: 112]
            callback_data=f"remsub_confirm_{i}" 
        )])
    
    if not buttons: # Should be caught by earlier check, but as a safeguard [cite: 112]
    ...
    await query.edit_message_text(f"Which of {person_name}'s subscriptions should we remove?",
                                   reply_markup=InlineKeyboardMarkup(buttons)) # [cite: 113]
    return REMSUB_CHOOSE_SUBSCRIPTION


async def remove_sub_confirm_and_delete(update: Update, context: ContextTypes.DEFAULT_TYPE): # Renamed [cite: 113]
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        await query.edit_message_text("Error: Invalid subscription selection. Please try again.") # [cite: 114]
        return REMSUB_CHOOSE_SUBSCRIPTION # Or end conversation [cite: 114]
    ...
    if person_name not in data["people"] or not data["people"][person_name].get("subscriptions") or sub_index >= len(data["people"][person_name]["subscriptions"]):
        await query.edit_message_text("Error: Subscription or person not found. It might have been removed already. Please try again.") # [cite: 115]
    ...
    try:
        ...
        slot_num_str = str(removed_sub_info.get("slot")) # Ensure it's a string for dict key matching [cite: 115]

        # Free up the slot in the account [cite: 115]
        if account_name and account_name in data["accounts"] and \
           slot_num_str and slot_num_str in data["accounts"][account_name]["slots"]: # [cite: 116]
            # Ensure we are freeing the slot only if this person occupied it [cite: 116]
            if data["accounts"][account_name]["slots"][slot_num_str] == person_name:
                data["accounts"][account_name]["slots"][slot_num_str] = None
                slot_freed_msg = f"Slot {slot_num_str} on account {account_name} is now free! 🎉" # [cite: 117]
            else:
                # This case should ideally not happen if data is consistent [cite: 117]
                ...
                logger.warning(f"Slot {slot_num_str} on {account_name} was expected to be held by {person_name} but found {data['accounts'][account_name]['slots'][slot_num_str]}") # [cite: 118]
        else:
        ...
        # Update person's last active date [cite: 118]
        data["people"][person_name]["last_active"] = datetime.utcnow().strftime("%Y-%m-%d")
        save_data(data)
        
        await query.edit_message_text( # [cite: 119]
...
{slot_freed_msg}""", parse_mode="Markdown"
        ...
        await query.edit_message_text("Error: Could not find that specific subscription to remove. It might have been removed already.") # [cite: 120]
    except Exception as e:
...
def main():
    ...
    # WEBHOOK_URL is optional for polling mode [cite: 121]
    # if not WEBHOOK_URL: [cite: 121]
    #     logger.critical("WEBHOOK_URL environment variable is not set for webhook mode.") [cite: 121]
    #     # raise RuntimeError("WEBHOOK_URL environment variable is not set.") # Only raise if webhook is mandatory [cite: 121]

    cleanup_expired_subs()  # Clean expired on startup [cite: 121]

    application = ApplicationBuilder().token(BOT_TOKEN).build()

    # Main menu buttons that are direct commands/actions, not conversation starts [cite: 121]
    ...
    # Conversation for adding a person [cite: 122]
    add_person_conv = ConversationHandler(
        ...
        map_to_parent={ConversationHandler.END: ConversationHandler.END} # Propagate END to potentially end parent if nested [cite: 122]
    )
    application.add_handler(add_person_conv)

    # Conversation for removing a person [cite: 122]
    remove_person_conv = ConversationHandler(
        ...
        states={CHOOSE_PERSON_TO_REMOVE: [CallbackQueryHandler(remove_person_confirm, pattern="^remove_person_.*")]}, # [cite: 123]
        fallbacks=[CommandHandler("cancel", cancel), CallbackQueryHandler(cancel, pattern="cancel"), CommandHandler("start", start)],
    )
    application.add_handler(remove_person_conv)

    # Add account conversation [cite: 123]
    add_account_conv = ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cancel), CallbackQueryHandler(cancel, pattern="cancel"), CommandHandler("start", start)], # [cite: 124]
    )
    application.add_handler(add_account_conv)

    # Remove account conversation [cite: 124]
    ...
    # Add subscription conversation (multi-step: Person -> Service -> Account -> Slot -> Duration) [cite: 124]
    ...
    application.add_handler(add_sub_conv) # [cite: 126]

    # Set prices conversation (main menu, add/edit, remove price option) [cite: 126]
    set_price_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(set_prices_start_cb, pattern="set_prices_start")],
        states={
            PRICE_MAIN_MENU: [
                CallbackQueryHandler(price_main_menu_handler), # Handles main_menu, view, add_edit_start, remove_start [cite: 126]
                # CallbackQueryHandler(set_prices_start_cb, pattern="set_prices_start_dummy") # Re-enter from view [cite: 126]
            ], # [cite: 127]
            ...
            PRICE_GET_DURATION_DAYS: [MessageHandler(filters.TEXT & ~filters.COMMAND, price_get_duration_days_receive)], # [cite: 128]
            PRICE_GET_PRICE_AMOUNT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, price_get_amount_receive),
                # Buttons from price_get_amount_receive to go back to PRICE_MAIN_MENU or main menu [cite: 128]
            ...
            # Removing a price option sub-flow [cite: 129]
    ...
    # Remove subscription conversation (direct command /removesub) [cite: 129]
    remove_sub_conv = ConversationHandler(
        entry_points=[CommandHandler("removesub", remove_sub_start_command)], # [cite: 130]
        states={
    ...
    # Global /start command to reset/show main menu (also used as a fallback) [cite: 130]
    application.add_handler(CommandHandler("start", start)) # Ensure this is added [cite: 130]

    # Other direct command handlers [cite: 131]
    application.add_handler(CommandHandler("listpeople", list_people)) # For direct command access [cite: 131]
    application.add_handler(CommandHandler("listsubs", list_subscriptions)) # For direct command access [cite: 131]
    application.add_handler(CommandHandler("income", calculate_income))
    application.add_handler(CommandHandler("export", export_data))
    application.add_handler(CommandHandler("setdefaultslots", set_default_slots)) # Renamed for consistency [cite: 131]
    application.add_handler(CommandHandler("addslot", add_slot)) # Renamed for consistency [cite: 131]
    application.add_handler(CommandHandler("removeslot", remove_slot)) # Renamed for consistency [cite: 131]

    # Start the bot
    # MODIFIED SECTION STARTS HERE
    if WEBHOOK_URL and "PORT" in os.environ:
        # Use a static webhook path for more robust deployment
        # Ensure your WEBHOOK_URL environment variable on Render does NOT have a trailing slash
        # e.g., https://your-app-name.onrender.com
        static_webhook_path = "telegramwebhook"  # You can change this, but keep it simple
        ...
        application.run_webhook(
            ...
            url_path=static_webhook_path,  # Using the static path
            webhook_url=full_webhook_url
        )
    # MODIFIED SECTION ENDS HERE
    else:
        logger.info("Starting polling")
        application.run_polling() # [cite: 133]
